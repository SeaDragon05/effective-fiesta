...
#include <unistd.h>
    ...
    #include <windows.h>
    // Equivalent functionality using Windows API
#else
    // Unix/Linux-specific code
    #include <sys/ioctl.h>
    // Original ioctl-related code
#endif
#include <locale>
#include <codecvt>
...
#include "GraphicalUserInterface.cpp"
#include "Castle.hpp"
...
int main(int argc, char **argv) {
	...
	if (general_settings_enable_TTS) {
    	...
    	resetColor();
    	//system("clear");
    	printf("\nTesting the compile of data...\n");
    	Database db;
    	Castle castle(db);
    	castle.compileData("res/beemoviescript.txt");
    	castle.compileData("res/copypasta.txt");
    	castle.compileData("res/copypasta2.txt");
    	castle.compileData("res/data.txt");
    	castle.compileData("res/skrek.txt");
    	castle.compileData("res/trumplmao.txt");

		if (LM_test_use_wikipedia) {
			for (int i = 0; i < 100; i++) {
				std::string dir = "res/1of2/wiki_" + ((i < 10) ? ("0" + std::to_string(i)) : std::to_string(i));
				castle.compileData(dir);
			}
			for (int i = 0; i < 71; i++) {
				std::string dir = "res/2of2/wiki_" + ((i < 10) ? ("0" + std::to_string(i)) : std::to_string(i));
				castle.compileData(dir);
			}
		}
    	int i = 0;
    	...
    	infoText.clear();
    	std::string line = "";
    	std::vector<std::shared_ptr<WordObject>> sentence = castle.generateSentence(0, 0);
