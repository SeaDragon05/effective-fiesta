...
import com.engine.graphics.ShadowShader;
...
public class ShadowMapEntityRenderer {
	 ...
	 *                             
	 *                             
	 *        projection matrix: contains the FOV
	 *        view matrix: contains the position and rotation
	 */
	public ShadowMapEntityRenderer(ShaderProgram shader, Matrix4f projectionViewMatrix) {
		this.shader = shader;
	...
	public void Spot_render(Map<TexturedModel, List<Entity>> entities) {
		for (TexturedModel model : entities.keySet()) {
			if (model == null)
				continue;
			RawModel rawModel = model.getRawModel();
	...
	private void Spot_prepareInstance(Entity entity) {
		if (entity == null) {
			return;
		}
		Matrix4f transformationMatrix = Maths.createTransformationMatrix(entity.getPosition(), entity.getRotX(),
				entity.getRotY(), entity.getRotZ(), entity.getScale());
		((SpotShadowShader) shader).loadTransformationMatrix(transformationMatrix);
	}
