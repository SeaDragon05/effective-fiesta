...
public class test {
	...
	public static void thingy() {
        ...
        // Define the Vendor ID and Product ID of the LEGO Mindstorms NXT device
        short vendorId = 0x0694;
        short productId = 0x0002;
        ...
        try {
            ...
            if (device == null) {
                throw new RuntimeException("LEGO Mindstorms NXT device not found");
            }
            ...
            // Data to send (example byte array)
             
            dataToSend = Main.playTone(500, 100); // Replace this with your actual data

            // Send the data to the USB device
            int transferred = LibUsb.controlTransfer(handle,
                    (byte) (LibUsb.REQUEST_TYPE_VENDOR | LibUsb.RECIPIENT_DEVICE | LibUsb.ENDPOINT_OUT),
                    (byte) 0x80, // bRequest (Vendor-specific request)
                    (short) 0x00, // wValue
                    (short) 0x00, // wIndex
                    dataToSend,
                    1000); // Timeout in milliseconds
