bl_info = {
    "name": "RealEngine.java",
    "author": "David Catuhe",
    "version": (1, 0),
    "blender": (2, 67, 0),
    "location": "File > Export > RealEngine.java (.java)",
    "description": "Export RealEngine.java scenes (.java)",
    "warning": "",
    "wiki_url": "",
    "tracker_url": "",
...
class Export_babylon(bpy.types.Operator, ExportHelper):  
   """Export RealEngine.java scene (.java)""" 
   bl_idname = "scene.java"
   bl_label = "Export RealEngine.java scene"
         ...
         for face in mesh.tessfaces:  # For each face
            ...
            for v in range(3): # For each vertex in face
               ...
               if (alreadySaved):
                  ...
                  vertices+="points[%.4f] = eng.pack(%.4f,%.4f,%.4f); \n"%(verticesCount,position.x,position.z,position.y)				
                  vertices+="normal[%.4f] = eng.pack(%.4f,%.4f,%.4f); \n"%(verticesCount,normal.x,normal.z,normal.y)
               ...
               if (v == 2):
                  indices+="p3 = %.4f;\n"%(index)
                  indices+="eng.GeometryAdd(points[p1], points[p2], points[p3], normal[p1], normal[p2], normal[p3], 0, cement);\n"
               indicesCount += 1
...
def menu_func(self, context):
   self.layout.operator(Export_babylon.bl_idname, text="RealEngine.java (.java)")
